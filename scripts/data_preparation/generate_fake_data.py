...
data = {
    ...
    'Koorts': [random.choice(['-', '0', '1']) for _ in range(num_samples)],
    'Hoesten': [random.choice(['-', '0', '1']) for _ in range(num_samples)],
    'Kortademigheid': [random.choice(['-', '0', '1']) for _ in range(num_samples)],
}
...
df.to_csv('data/raw/patient_data/example_dataset.csv', index=False)